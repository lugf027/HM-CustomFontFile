...
public:
    ...
    ~PluginRender()
    {
        if (sampleGraphics_ != nullptr) {
            sampleGraphics_->Release(id_);
            delete sampleGraphics_;
            sampleGraphics_ = nullptr;
        }
    }
    void DrawPattern(const char* fontPath, const char * content, double fontHeight);
    int32_t HasDraw();
...
private:
    SampleGraphics* sampleGraphics_;
    int64_t id_;
    int32_t hasDraw_;
