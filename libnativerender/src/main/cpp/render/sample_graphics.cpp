...
static void NativeOnDrawText(OH_Drawing_Canvas *canvas, const char *fontPath,const char *content, double fontHeight) {
    // 选择从左到右/左对齐等排版属性
...
//    TEXT_BASELINE_IDEOGRAPHIC
    ...
    // 设置字体类型等
    OH_Drawing_Matrix *matrix = OH_Drawing_MatrixCreateTranslation(0, adaptationUtil->GetHeight(30.f));
    // 对Canvas进行矩阵变换
    ...
    // 将文本绘制到画布上
    OH_Drawing_CanvasDrawLine(canvas, x, y, x + 100, y);
...
void SampleGraphics::DrawMyTest(OH_Drawing_Canvas *canvas, const char *fontPath, const char *content, double fontHeight) {
    ...
    NativeOnDrawText(canvas, fontPath, content, fontHeight);
}
